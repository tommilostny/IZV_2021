...
def plot_stat(data_source:Dict[str, np.ndarray], fig_location:str=None, show_figure:bool=False) -> None:
    ...
    #Uložit statistiky (počty hodnot ve sloupci p24) pro každý region.
    ...
    #Nastavit barvu (bílá pro políčka bez nehody).
    cmap = plt.get_cmap("viridis").copy()
    cmap.set_under("white")
    
    #Absolutní statistika v logaritmickém měřítku.
    im1 = axs[0].imshow(stats_abs, cmap=cmap, norm=colors.LogNorm(vmin=1))

    #Relativní statistika v lineárním měřítku, v % hodnoty 0-100.
    im2 = axs[1].imshow(stats_rel, cmap=cmap, vmin=0.01, vmax=100)

    #Nastavení popisků os obou grafů.
    for ax in axs:
    ...
    #Legenda, barevné spektrum.
    plt.colorbar(im1, shrink=0.6, ax=axs[0], label="Počet nehod")
    plt.colorbar(im2, shrink=0.6, ax=axs[1], label="Podíl nehod pro danou příčinu [%]")
